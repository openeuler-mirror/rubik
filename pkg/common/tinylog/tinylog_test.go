...
import (
	...
	"isula.org/rubik/pkg/common/try"
)
...
func TestInitConfigLogDriver(t *testing.T) {
	...
	err = InitConfig("file", logDir, "", logSize)
	assert.NoError(t, err)
	assert.Equal(t, logFile, logDriver)
	...
	assert.NoError(t, err)
	assert.Equal(t, logStdio, logDriver)
...
func TestInitConfigLogSize(t *testing.T) {
	...
	for i := 0; i < printLine; i++ {
		Logf(strings.Repeat("TestInitConfigLogSize log", repeat))
	}
